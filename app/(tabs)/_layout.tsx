...
export default function HomeScreen() {
  ...
  const handleLogin = () => {
    // Basic verification matching local persistent layer rules
    ...
    setToastConfig({ message: 'Invalid Username/Phone or Password.', type: 'error' });
        ...
        <View style={[styles.formContainer, { backgroundColor: cardColor }]}>
          ...
          <View style={[styles.inputWrapper, { backgroundColor: inputBg }]}>
            <TextInput 
              ...
              value={identifier}
              onChangeText={(text) => {
                // If the user inputs numbers, filter to protect the 11-digit syntax layout rule
                if (/^\d+$/.test(text) || text === '') {
